...
(function task3() {
    ...
    // const iterator = generator(1, 100);
...
(function task3(buttonId: string): void {
    ...
    // const stream$ = fromEvent(target, 'click').pipe(
...
(function task6() {
    ...
    //     map(data => data.response.map(el => el.name)),
    //     switchMap(arr => from(arr)),
    // );
    //
    // run(stream$);
...
(function task10() {
    ...
    //         }, [])
    //     )),
    //     filter(([el, fields]) => fields.indexOf(el[0]) > -1),
...
(function task11() {
    ...
    const stream$ = from(items).pipe(
        concatMap(item => concat(of(item), pause)),
    );
...
// Task 18. HW
// Используя 3 функции создания потоков на выбор создать потоки и применить минимальное преобразование для них,
